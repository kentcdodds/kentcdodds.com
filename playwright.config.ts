...
export default defineConfig({
	...
	webServer: {
		command: process.env.CI
			? `cross-env PORT=${PORT} PLAYWRIGHT_TEST_BASE_URL=http://localhost:${PORT} bun run start:mocks`
			: `cross-env PORT=${PORT} PLAYWRIGHT_TEST_BASE_URL=http://localhost:${PORT} bun run dev`,
		port: Number(PORT),
