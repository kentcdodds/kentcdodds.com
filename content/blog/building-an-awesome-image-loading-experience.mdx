...
<video
  ...
  loop={true}
  muted={true}
  src="https://res.cloudinary.com/kentcdodds-com/video/upload/kentcdodds.com/content/blog/building-an-awesome-image-loading-experience/blurred-image-load_zzqqnt.mp4"
/>
...
only loading the size of image that you need. If you've got an images that's
...
just use the `src` attribute like ususual.
...
loads quick and I save you data!
...
<video
  ...
  loop={true}
  muted={true}
  src="https://res.cloudinary.com/kentcdodds-com/video/upload/kentcdodds.com/content/blog/building-an-awesome-image-loading-experience/unsplash-loading_l2ekrr.mp4"
/>
...
canvas appraoch is the size of the data required for the image is really small.
...
converted into a hash and the canvas represenation of
...
blurhash. Once you apply this to just a couple images, blurhash more than pays
for its own weight. So a site like unsplash definitely hits that point where it
...
ultimate image we want loaded. We accept a `blurDataUrl` to render a blurred
version of the image while we're waiting for the image to load. And then the
rest of the props are just applied to the `div` that's the container for
...
Then if there is a `blurDataUrl`, we render the an `img` element with the
...
loaded and when it does finishe loading we'll trigger an update to make the
image fade in.
