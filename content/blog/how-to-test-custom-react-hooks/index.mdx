...
<callout-warning>
  `@testing-library/react`'s latest version has a `renderHook` utility which you
  can use now (instead of `@testing-library/react-hooks`). If anyone wants to
  update this blog post to use that instead you'll find a link to edit the post
