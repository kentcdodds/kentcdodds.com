...
date: 2022-03-24
description:
  Remix fills the missing piece that all React web applications need. Let me
  explain.
categories:
  - remix
  - react
...
I've been building React applications since 2015. Since then, React has been the
...
fact that much of the state we're manging is actually a cache and suffers from
...
managing this problem, as evidenced by the myrid of libraries and tools that
...
libraries, we shoot a grapling hook over the network chasm via HTTP to get data
...
The code required for this grapling hook to work all exists on the frontend. For
data fetching, you have to know what data to fetch, and often that's a
...
existing backed you can certainly stick with that.
