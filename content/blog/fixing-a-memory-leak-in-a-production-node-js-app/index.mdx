...
![Screenshot of the Fly metrics UI on a mobile device showing two firecracker graphs, one for load average and one for memory usage. There are two clear spikes of each and it looks not so good.](/media/kentcdodds.com/content/blog/fixing-a-memory-leak-in-a-production-node-js-app/two-spikes?tr=f_auto,q_auto,dpr_2.0,w_1600)
