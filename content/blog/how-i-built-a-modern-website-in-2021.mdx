...
At the time of this writting (September 2021) here are the
...
github.com/AlDanial/cloc v 1.90  T=0.28 s (883.8 files/s, 103677.4 lines/s)
-------------------------------------------------------------------------------
Language                     files          blank        comment           code
-------------------------------------------------------------------------------
TypeScript                     217           1955            573          20603
CSS                             11            195            299           4655
JSON                             7              0              0            609
YAML                             2             43             13            232
SQL                              7             20             25             52
...
TOML                             1              0              2              1
-------------------------------------------------------------------------------
SUM:                           250           2215            915          26196
-------------------------------------------------------------------------------
```

26k lines of code isn't like your work project where a half dozen teams have
...
biggest Remix application in existance at the moment.
...
Most development has taken place in the last 3-4 months. There are ~825 commits
so far 😅.
...
- Reach UI: A set of accessibile UI components every app needs
  (accordion/tas/dialog/etc...)
- ESBuild: JavaScript bundler (used by Remix and mdx-bundler).
- mdx-bundler: Tool for compiling and bundling MDX for my site content (blog
  posts and some simple pages).
- Octokit: Library making interacting with the GitHub API easier.
- Framer Motion: Great React Animation library
- Unified: Markdown/HTML parser/tranformer/compiler system.
...
changes that ocurred between the commit that's being built and the commit of the
last time there was a refresh (that value is stored in redis and my server
exposes an endpoint for my action to retrieve it). If any of the changed files
were in the `./conent` directory, then the action makes an authenticated POST
...
This reduces what used to take 10-25 minutes down to 20 seconds. And it saves me
...
One of the coolest parts of Fly.io (and the region I chose Fly over alternative
...
To really get the network performance beneifts of colocation, you need your data
...
What's more, I don't have to make the trade-off of vender lock-in. At any time I
...
multi-regional deployment comes with the problem if consistency. I've got
...
<Themed
  dark={
    ...
    >
      <BlogImage
        cloudinaryId="kentcdodds.com/content/blog/how-i-built-a-modern-website-in-2021/fly-request-replays-dark.png"
        imgProps={{
...
rest of the request performs it's reads against the primary database.
...
Fly.io. It's phenominal. But I've built my own little abstraction for
...
- `forceFresh` is what you think. It skips looking at the cache and will call
  `getFreshValue` even if the value hasn't expired yet.
...
all. All the backend stuff is outside of my wheel house. What's funny though is
...
these migrations much simpler for me, an guy who hasn't done SQL in years and
...
system even if it's implicit because the "reset password" flow which emails you
a link to reset your password. So it's certainly not any less secure. In fact,
actually more secure because there's no password to lose.
...
![Remix logo](https://res.cloudinary.com/kentcdodds-com/image/upload/kentcdodds.com/content/blog/how-i-built-a-modern-website-in-2021/remix.png)
