...
npm ERR! code E403
npm ERR! 403 403 Forbidden - GET https://npm.remix.run/@remix-run%2freact - Missing authorization
...
```sh
npm install remix @remix-run/react @remix-run/serve
```
...
include the script tag. Remix conveniently gives us a component we can
render to render that script tag:

```tsx lines=3,15 filename=app/root.jsx
import * as React from 'react'
