...
reset your component to it's original state without having to completely unmount
...
**With the state initializer pattern** (notice there's no animation):
...
**With the key reset approach** (notice there is an animation):
