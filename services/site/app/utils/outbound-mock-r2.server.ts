...
function getOrCreateBucketStore(bucket: string) {
	...
	if (!store) {
		...
		try {
			const semanticSearchBucket = getEnv().R2_BUCKET?.trim() || null
			if (semanticSearchBucket && bucket === semanticSearchBucket) {
				const ignoreListKey =
					typeof getEnv().SEMANTIC_SEARCH_IGNORE_LIST_KEY === 'string' &&
					getEnv().SEMANTIC_SEARCH_IGNORE_LIST_KEY.trim()
						? getEnv().SEMANTIC_SEARCH_IGNORE_LIST_KEY.trim()
			...
			// Env may not be initialized in some unit-test contexts.
		}
...
export async function maybeHandleR2MockFetch(request: Request) {
	const url = new URL(request.url)
	if (!isR2Host(url)) return null
	if (!shouldMockR2(request)) return null
	...
	if (request.method === 'GET' && url.searchParams.get('list-type') === '2') {
		...
		return xmlResponse(listObjectsV2Xml({ bucket, prefix, keys, store }))
	}
	...
	if (request.method === 'GET') {
		...
		if (range) {
			...
			if (
				...
				return new Response(bodyInitFromBytes(chunk), {
					status: 206,
					headers: {
						'Content-Type': obj.contentType,
		...
		return new Response(bodyInitFromBytes(obj.body), {
			status: 200,
			headers: {
				'Content-Type': obj.contentType,
				'Content-Length': String(obj.size),
