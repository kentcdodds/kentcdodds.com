...
export default {
	async fetch(
		...
		if (url.pathname === '/__mocks/meta') {
			return jsonResponse({
				service: 'verifier',
				description: 'Mock VerifyRight API worker',
				dashboard: '/__mocks',
		...
		if (request.method === 'GET' && /^\/verify\/[^/]+$/.test(url.pathname)) {
			const email = decodeURIComponent(url.pathname.split('/')[2] ?? '')
			if (!email.includes('@') || email.endsWith('@invalid.test')) {
				return jsonResponse(
					{
						status: false,
						error: {
...
<html lang="en">
  <head>
    ...
    <title>Verifier Mock Dashboard</title>
    <style>
      :root {
  ...
  <body>
    <main class="layout">
      <h1>Verifier Mock Dashboard</h1>
      <p class="summary">
        Recent requests for the VerifyRight mock worker.
      </p>
      <div class="toolbar">
