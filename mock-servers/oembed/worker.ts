...
export default {
	async fetch(
		...
		if (url.pathname === '/__mocks/meta') {
			return jsonResponse({
				service: 'oembed',
				description: 'Mock oEmbed provider registry and endpoint worker',
				dashboard: '/__mocks',
		...
		if (request.method === 'GET' && url.pathname === '/providers.json') {
					...
					endpoints: [
						{
							...
							url: `${origin}/providers/codesandbox`,
						},
					],
				},
				{
		...
		if (request.method === 'GET' && url.pathname === '/providers/twitter') {
			const targetUrl = url.searchParams.get('url') ?? ''
			return jsonResponse({
				html: `<blockquote class="twitter-tweet" data-dnt="true" data-theme="dark"><p lang="en" dir="ltr">Mock oEmbed response for ${escapeHtml(targetUrl)}</p></blockquote>`,
			})
		}

		if (request.method === 'GET' && url.pathname === '/providers/codesandbox') {
			return jsonResponse({
				html: '<iframe width="1000" height="500" src="https://codesandbox.io/embed/mock?view=editor" sandbox="allow-modals allow-forms allow-popups allow-scripts allow-same-origin" style="width: 1000px; height: 500px; border: 0px; border-radius: 4px; overflow: hidden;"></iframe>',
			})
		}

		if (request.method === 'GET' && url.pathname === '/providers/youtube') {
			return jsonResponse({
				html: '<iframe width="200" height="113" src="https://www.youtube.com/embed/dQw4w9WgXcQ?feature=oembed" frameborder="0" allowfullscreen></iframe>',
			})
		}
...
<html lang="en">
  <head>
    ...
    <title>oEmbed Mock Dashboard</title>
    <style>
  ...
  <body>
    <main class="layout">
      <h1>oEmbed Mock Dashboard</h1>
      <p class="summary">Provider registry and endpoint request history.</p>
      <div class="toolbar">
