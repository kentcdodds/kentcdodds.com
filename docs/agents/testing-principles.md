...
- Prefer behavior assertions over implementation details. Avoid pinning CSS
  ...
  contract under test.
- Run backend unit tests with `bun run --filter kentcdodds.com test:backend`
  (or `bun run test:backend:watch` while iterating). Site-worker tests:
  `bun run --filter site-worker test`.
